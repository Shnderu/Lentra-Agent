...
from lentra.core.risk.risk_engine import RiskEngine


class IngestionPipeline:
    """
    VIETNAM PIPELINE:
    INGESTION + DEDUP + PRICING + RISK
    """
    ...
    def ingest_batch(self, raw_items: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # 2. dedup
        dedup_result = self.deduper.deduplicate(normalized_items)
        clusters = dedup_result["clusters"]

        # 3. pricing
        market_stats = self.pricing.build_market(clusters)
        ...
        for cluster in clusters:
            ...
            for item in cluster:
                price_eval = self.pricing.evaluate(item, market_stats)
                ...
                enriched.append({
                    **item_with_price,
                    ...
                    "risk_signals": risk_eval["signals"]
                })

        return {
            "ingested": len(normalized_items),
